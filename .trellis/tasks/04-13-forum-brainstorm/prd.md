...
构建一个 AI + 生活风格的**纯文字论坛**，部署在 Deno Deploy 上。 使用 Deno Fresh
2.0 框架 + Deno KV 数据库，提供类似 NodeSeek / DeepFlood 的帖子列表式体验。
...
1. **Key 设计支持排序**：时间戳使用反转值（`MAX_TIMESTAMP - timestamp`）使
   `kv.list()` 的字典序等于时间倒序
2. **搜索采用简单的逻辑**：对标题分词后建立倒排索引，适合 KV
   存储，不需要外部搜索引擎
