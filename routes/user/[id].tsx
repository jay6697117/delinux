    ...
    <div style={{ padding: "var(--space-md) 0" }}>
      <div class="card">
        <div class="user-header">
          <div class="user-avatar">{profileUser.username.charAt(0).toUpperCase()}</div>
          <div class="user-info">
            <h1>
              {profileUser.username}
              {profileUser.role === "admin" && <span class="user-badge admin" style={{ marginLeft: "8px" }}>管理员</span>}
              {profileUser.banned && <span class="user-badge banned" style={{ marginLeft: "8px" }}>已禁言</span>}
            </h1>
      ...
      <div class="tabs" style={{ marginTop: "var(--space-md)" }}>
        <a href={`/user/${profileUser.id}?tab=posts`} class={`tab ${tab === "posts" ? "active" : ""}`}>📝 帖子</a>
        {isOwner && <a href={`/user/${profileUser.id}?tab=favorites`} class={`tab ${tab === "favorites" ? "active" : ""}`}>⭐ 收藏</a>}
      </div>
      <div class="card">
        {posts.length === 0 ? (
          <div class="empty-state"><div class="empty-state-icon">{tab === "posts" ? "📝" : "⭐"}</div><p class="empty-state-text">{tab === "posts" ? "还没有发过帖子" : "还没有收藏的帖子"}</p></div>
        ) : (
          <ul class="post-list">
            {posts.map((post) => (
