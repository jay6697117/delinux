...
export const handler = define.handlers({
  GET(ctx) {
    ...
    return { data: { error: "", board: url.searchParams.get("board") || "", title: "", content: "" } };
  },
  async POST(ctx) {
    if (!ctx.state.user) return new Response(null, { status: 302, headers: { location: "/auth/login" } });
    ...
    if (!board) error = "请选择版块";
      ...
      return new Response(null, { status: 302, headers: { location: `/post/${post.id}` } });
    }
          ...
          <form method="POST">
            <div class="form-group">
              ...
              <select class="form-input" id="board" name="board" required>
                ...
                {BOARDS.map((b) => <option value={b.slug} selected={b.slug === board}>{b.icon} {b.name}</option>)}
              </select>
            </div>
            <div class="form-group">
              <label class="form-label" for="title">标题</label>
              <input class="form-input" type="text" id="title" name="title" value={title} placeholder="一句话概括你想说的" required autofocus />
            </div>
            <div class="form-group">
              <label class="form-label" for="content">内容</label>
              <textarea class="form-input" id="content" name="content" placeholder="支持 Markdown 格式" required>{content}</textarea>
              <p class="form-hint">支持 Markdown 语法：**粗体**、*斜体*、`代码`、&gt; 引用、- 列表</p>
            </div>
            <button type="submit" class="btn btn-primary" style={{ width: "100%" }}>发布帖子</button>
          </form>
        </div>
      </div>
    </div>
