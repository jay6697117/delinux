...
export default define.page<typeof handler>(
  function PostDetail({ data, state }) {
    const { post, board, replies, htmlContent, liked, favorited, replyError } =
      data;
    return (
      <div style={{ padding: "var(--space-md) 0" }}>
        <div class="card">
          <div class="post-detail">
            ...
            <div class="post-detail-meta">
              <a href={`/user/${post.authorId}`} class="post-author">
                {post.authorName}
              </a>
          ...
          {state.user && (
            <div class="action-bar">
              ...
              >
                ...
                <button
                  type="submit"
                  class={`action-btn ${favorited ? "active" : ""}`}
                >
                  {favorited ? "⭐ 已收藏" : "☆ 收藏"}
                </button>
              </form>
            ...
            : replies.map((reply, i) => (
              <div class="reply-item" key={reply.id}>
                <div class="reply-header">
                  <a href={`/user/${reply.authorId}`} class="reply-author">
                    {reply.authorName}
                  </a>
        ...
        {state.user
          ? (
            <div class="card" style={{ marginTop: "var(--space-md)" }}>
              <div class="card-body">
                {replyError && <div class="alert alert-error">{replyError}
                </div>}
                <form method="POST">
